...
export function getSysId(field: unknown): string {
  ...
  if (typeof field === 'object' && field !== null) {
    ...
    // If value is also an object, try to extract from it recursively
    if (typeof value === 'object' && value !== null) {
      return getSysId(value);
    }
    return '';
  }
...
export function getDisplayValue(field: unknown): string {
  ...
  if (typeof field === 'object' && field !== null) {
    const ref = field as ReferenceField;
    // Ensure we return a string - check each property type
    if (typeof ref.display_value === 'string' && ref.display_value) {
      return ref.display_value;
    }
    if (typeof ref.value === 'string' && ref.value) {
      return ref.value;
    }
    // If properties are also objects, try to extract recursively
    if (ref.display_value && typeof ref.display_value === 'object') {
      return getDisplayValue(ref.display_value);
    }
