...
export interface DuplicateResolutionAction {
  pairId: string;
  action: 'merge' | 'delete' | 'keep' | 'skip';
  targetSysId?: string; // For merge: which record to keep
  deleteSysId?: string; // For delete: which record to remove
  notes?: string;
}
