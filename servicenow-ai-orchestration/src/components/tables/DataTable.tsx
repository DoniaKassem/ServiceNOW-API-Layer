...
export function DataTable({
  ...
  // Canonical row IDs for the currently loaded page (keep selection logic consistent everywhere)
  const pageRowIds = useMemo(() => {
    return data.map((row, idx) => getSysId(row.sys_id) || `row-${idx}`);
  }, [data]);
  ...
  const handleSelectAll = useCallback(() => {
    ...
    if (allSelectedOnPage) {
      ...
      selectAllRows(pageRowIds);
    }
  }, [pageRowIds, selectedRows, selectAllRows, clearSelection]);
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                data.map((row, rowIndex) => {
                  const rowId = pageRowIds[rowIndex] || `row-${rowIndex}`;
